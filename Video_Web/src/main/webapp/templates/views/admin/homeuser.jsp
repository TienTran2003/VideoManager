...
<body class="hold-transition sidebar-mini layout-fixed">
							...
							<tbody>
								<c:forEach items="${user}" var="item">
									<tr>
										...
										<td>${item.isAdmin}</td>
										<td>
											<button type="button" onClick="deleteUser('${item.username}')" class="btn btn-block bg-gradient-danger btn-sm">Delete</button>
										</td>
									</tr>
								</c:forEach>
							</tbody>
	...
	<script>
		function deleteUser(username) {
			$.ajax({
				url: '/PS24803_TranVanTien_ASM/admin/user?action=delete&username=' + username
			}).then(function(data) {
				window.lacation.href = "http://localhost:8080/PS24803_TranVanTien_ASM/admin/user";
			}).fail(function(error) {
